...
/**
 * InterviewV2 - BACKEND AI PROBING ONLY
 * Deterministic base questions + follow-up packs (UI-driven)
 * AI probing handled ENTIRELY by backend function (no front-end agent calls)
...
export default function InterviewV2() {
  ...
  // Backend AI probing state (NO FRONT-END AGENT)
  const [backendProbeMessages, setBackendProbeMessages] = useState([]);
  const [isWaitingForBackendProbe, setIsWaitingForBackendProbe] = useState(false);
  ...
  // Track global display numbers for questions
  ...
  useEffect(() => {
    if (!isCommitting) {
      requestAnimationFrame(() => {
        if (isWaitingForBackendProbe && inputRef.current) {
          inputRef.current.focus({ preventScroll: false });
        } else if (currentItem && !isWaitingForBackendProbe) {
  ...
  }, [currentItem, isCommitting, isWaitingForBackendProbe]);

  useEffect(() => {
    if (transcript.length > 0 || backendProbeMessages.length > 0) {
      setTimeout(autoScrollToBottom, 150);
    }
  }, [transcript.length, backendProbeMessages.length, autoScrollToBottom]);
  ...
  const initializeInterview = async () => {
    try {
      console.log('🚀 [InterviewV2] Initializing BACKEND-ONLY interview flow...');
      console.log('   - Session ID from URL:', sessionId);
      const startTime = performance.now();

      // Step 1: Load session with validation
      console.log('📡 [InterviewV2] Fetching session from database...');
      const loadedSession = await base44.entities.InterviewSession.get(sessionId);
      
      if (!loadedSession) {
        console.error('❌ [InterviewV2] Session not found in database');
        throw new Error(`Session not found: ${sessionId}. It may have been deleted or never created.`);
      }
      
      if (!loadedSession.id) {
        console.error('❌ [InterviewV2] Session object missing ID field:', loadedSession);
        throw new Error('Invalid session object returned from database');
      }
      
      console.log('✅ [InterviewV2] Session loaded successfully');
      ...
      console.log('⚙️ [InterviewV2] Bootstrapping engine...');
      const engineData = await bootstrapEngine(base44);
      console.log('✅ [InterviewV2] Engine bootstrapped');
      setEngine(engineData);
      
      // NO AGENT INITIALIZATION - BACKEND ONLY
      console.log('🤖 [InterviewV2] Skipping front-end agent initialization - using backend probing only');
      
      // Step 3: Restore state from snapshots or rebuild from responses
      ...
      if (needsRebuild) {
        console.log('🔧 [InterviewV2] Session needs rebuild - rebuilding from Response entities...');
        await rebuildSessionFromResponses(engineData, loadedSession);
      } else if (hasValidSnapshots) {
        console.log('🔄 [InterviewV2] Restoring from session snapshots...');
        restoreFromSnapshots(engineData, loadedSession);
      } else {
        console.log('🎯 [InterviewV2] Starting fresh interview');
      ...
      console.log(`✅ [InterviewV2] Backend-only interview ready in ${elapsed.toFixed(2)}ms`);

    } catch (err) {
      console.error('❌ [InterviewV2] Initialization failed:', err);
  ...
  // ============================================================================
  // NEW: BACKEND AI PROBING (REPLACES FRONT-END AGENT)
  // ============================================================================

  const triggerBackendProbe = async (questionId, packId, followUpAnswers) => {
    console.log(`🤖 [Backend Probe] Triggering backend AI probe for ${questionId}/${packId}...`);
    ...
    setBackendProbeMessages(prev => [...prev, thinkingBubble]);
    
    try {
      // Build context for backend
      const question = engine.QById[questionId];
      const packSteps = injectSubstanceIntoPackSteps(engine, packId, followUpAnswers.find(a => a.questionText?.includes('substance'))?.answer);
      ...
      followUpAnswers.forEach((answer) => {
        contextLines.push(`- ${answer.questionText}: ${answer.answer}`);
      });
      
      contextLines.push(``);
      contextLines.push(`Full Transcript History:`);
      transcript.forEach((entry) => {
        if (entry.type === 'question') {
          contextLines.push(`Q: ${entry.questionText} → ${entry.answer}`);
        }
      });
      
      const promptPayload = contextLines.join('\n');
      
      console.log('📤 [Backend Probe] Calling backend function...');
      ...
      console.log('✅ [Backend Probe] Backend probe succeeded');
      console.log('   - Result:', response.data?.result);
      
      // Remove thinking bubble
      setBackendProbeMessages(prev => prev.filter(m => m.id !== thinkingBubble.id));
      
      // Add investigator probe bubble
      if (response.data?.result) {
        ...
        setBackendProbeMessages(prev => [...prev, probeBubble]);
      }
      ...
      console.error('❌ [Backend Probe] Backend probe failed:', err);
      console.error('   - Error message:', err?.message);
      
      // Remove thinking bubble on error
      setBackendProbeMessages(prev => prev.filter(m => m.id !== thinkingBubble.id));
      
      // DO NOT stop interview - just skip probing and continue
      return false;
    }
  };
  ...
  // ============================================================================
  // ANSWER SUBMISSION - DETERMINISTIC FLOW WITH BACKEND AI PROBING
  // ============================================================================
      ...
      if (currentItem.type === 'question') {
        ...
        if (isLastFollowUp) {
          ...
          if (shouldSkipProbingForHired(packId, updatedFollowUpAnswers)) {
            ...
            // NEW: Trigger backend AI probing
            ...
            if (triggeringQuestion) {
              ...
              // Call backend probe
              await triggerBackendProbe(
                triggeringQuestion.questionId,
                packId,
                packAnswers
              );
              
              // Advance to next question after probing
              await advanceToNextBaseQuestion(triggeringQuestion.questionId);
            } else {
              console.error(`❌ Could not find triggering question for pack ${packId} when trying to trigger backend probe. Marking interview complete.`);
              setCurrentItem(null);
              setQueue([]);
              await persistStateToDatabase(newTranscript, [], null);
              setShowCompletionModal(true);
  ...
  }, [currentItem, engine, queue, transcript, sessionId, isCommitting, currentFollowUpAnswers, advanceToNextBaseQuestion]);
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-lg font-semibold text-white">ClearQuest Interview (V2)</h1>
              </div>
              <Button
                variant="outline"
                size="sm"
        ...
        <main className="flex-1 overflow-hidden flex flex-col">
          ...
          >
            <div className="max-w-5xl mx-auto space-y-4">
              ...
              {/* Show backend probe messages */}
              {backendProbeMessages.length > 0 && (
                <div className="space-y-4 border-t-2 border-purple-500/30 pt-4 mt-4">
                  ...
                  {backendProbeMessages.map((msg) => (
                    <BackendProbeMessageBubble key={msg.id} message={msg} />
                  ))}
                </div>
              )}
            </div>
...
// Backend probe message bubbles
function BackendProbeMessageBubble({ message }) {
  if (message.type === 'thinking') {
    return (
      <div className="flex justify-center">
        <div className="bg-slate-700/50 rounded-xl px-5 py-3 flex items-center gap-2">
          <Loader2 className="w-4 h-4 text-slate-400 animate-spin" />
          <p className="text-slate-300 text-sm italic">{message.content}</p>
        </div>
      </div>
    );
  }
