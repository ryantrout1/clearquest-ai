    ...
    <div className="h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto cq-scroll scrollbar-thin" ref={historyRef} onScroll={handleTranscriptScroll}>
        <div className="px-4 pt-6 flex flex-col min-h-full justify-end" style={{ paddingBottom: `${24 + dynamicBottomPaddingPx}px` }}>
          ...
          {(() => {
            // PART A: Build single canonical render stream from three sources
            // Step 1: Transcript (dedupedby existing logic)
            const transcriptRenderable = renderedTranscriptSnapshotRef.current || renderedTranscript;
            
            // Step 2: V3 UI history (already deduped by stableKey in handleV3AnswerSubmit)
            const v3UiRenderable = v3ProbeDisplayHistory || [];
            
            // Step 3: Active card (exactly ONE, derived from activeUiItem.kind precedence)
            let activeCard = null;
            ...
            // PART E: Stream snapshot log (only on length changes)
            if (renderStream.length !== lastRenderStreamLenRef.current) {
              console.log("[STREAM][SNAPSHOT]", {
                len: renderStream.length,
              ...
              lastRenderStreamLenRef.current = renderStream.length;
            }
