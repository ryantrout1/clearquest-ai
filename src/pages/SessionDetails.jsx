...
export default function SessionDetails() {
  ...
  const loadSessionData = async () => {
    ...
    try {
      ...
      try {
        const summariesResult = await base44.functions.invoke('getSessionSummaries', {
          session_id: sessionId
        });

        if (summariesResult.data.ok) {
          const { questionSummaries, sectionSummaries } = summariesResult.data;

          console.log('[SESSIONDETAILS] Loaded QuestionSummaries', {
            sessionId,
            count: questionSummaries.length,
            sample: questionSummaries.slice(0, 3).map(q => ({
              question_id: q.question_id,
              summaryLength: q.question_summary_text?.length || 0
            }))
          });

          console.log('[SESSIONDETAILS] Loaded SectionSummaries', {
            sessionId,
            count: sectionSummaries.length,
            sample: sectionSummaries.slice(0, 3).map(s => ({
              section_id: s.section_id,
              summaryLength: s.section_summary_text?.length || 0
            }))
          });
  ...
  const allResponsesWithNumbers = responses.map((r, idx) => {
    ...
    // Augment with summary from QuestionSummary entity
    const questionSummary = questionSummariesByQuestionId[r.question_id];
    const investigator_summary = r.investigator_summary || questionSummary || null;
  ...
  // Auto-generate AI summaries on load if needed
  useEffect(() => {
    if (!sessionId || autoGeneratedRef.current) return;
    if (!session || responses.length === 0) return;

    const hasGlobalAISummary = !!session.global_ai_summary;
    const hasSectionAISummaries = !!session.section_ai_summaries && Object.keys(session.section_ai_summaries).length > 0;
    
    const responsesWithFollowups = responses.filter(r => 
      r.answer === 'Yes' && followups.some(f => f.response_id === r.id)
    );
    
    const hasQuestionNeedingSummary = responsesWithFollowups.some(r => !r.investigator_summary);

    const shouldAutoGenerate = 
      (!hasGlobalAISummary || !hasSectionAISummaries || hasQuestionNeedingSummary) &&
      responsesWithFollowups.length > 0;

    if (!shouldAutoGenerate) {
      console.log('[AI-AUTO] SKIP', {
        sessionId,
        hasGlobalAISummary,
        hasSectionAISummaries,
        hasQuestionNeedingSummary,
        responsesWithFollowupsCount: responsesWithFollowups.length
      });
      return;
    }

    autoGeneratedRef.current = true;
    console.log('[AI-AUTO] START', {
      sessionId,
      hasGlobalAISummary,
      hasSectionAISummaries,
      hasQuestionNeedingSummary,
    ...
    handleGenerateAllAISummaries()
      .then(() => {
        console.log('[AI-AUTO] DONE', { sessionId });
      })
      .catch((err) => {
        console.error('[AI-AUTO] ERROR', { sessionId, error: err });
      });
  }, [sessionId, session?.id, responses, followups]);
